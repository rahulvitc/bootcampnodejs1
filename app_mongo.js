...
app.get("/catname/:name/age/:age", function(req, res) {
    //console.log(req.params);
    ...
    console.log("catname: " + catname + " age: " + age);
    var george = new Cat({
        name: catname,
        age: age,
        temperament: "Evil8"
...
app.get("/getcatdetails",function(req,res){
    Cat.find({}, function(err, cats){
        if (err) {
            console.log("err find");
        }else{
            ...
            //console.log(cats);
            //res.send(cats);
            var name = req.params.cats;
            console.log("name:"+name);
            res.render("catdetails.ejs", { NAME: name });
        }
    });
});
